...
char showmenu(void)
{
    ...
    while (strchr("ulton", ans) == NULL)//如果有该ans字符，那么返回ulton的地址（指针）且跳出循环；没有则返回NULL进入循环
    {
        puts("Please enter a u, l, t, o, or n:");//循环中的处理
        ans = tolower(getchar());
        eatline();
    }
    return ans;
...
void eatline(void)
{
    while (getchar() != '\n')//输入字符不为回车那么continue，是回车跳过结束循环
    continue;
}

void ToUpper(char * str)
{
    while (*str)//只要指针所指不是空字符，将支付转化为大写。
    {
        *str = toupper(*str);
        str++;//相当于最后的空字符跳出来，没什么问题
    }
}
